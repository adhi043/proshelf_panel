...
const EditProduct = () => {
    ...
    useEffect(() => {
        try {
            ...
            axios.get(`${BaseUrl.baseUrl}/product/get/${id}`).then(res => {
                if (res.data.status === 'ok') {
    ...
    const submit = () => {
            ...
            try {
                ...
                axios.put(`${BaseUrl.baseUrl}/product/update/${id}`,param).then(res => {
                    if (res.data.status === 'ok') {
                ...
                <div className='col-md-12 mb-4'>
                    ...
                    <div className='col-md-6 mb-4'>
                        <div className='border border-dark rounded-3 bg-white d-flex align-items-center px-3'>
                            ...
                            <input type='text' className='border-0 rounded-3 p-2 w-100 px-3' placeholder='Enter Product Name' defaultValue={name} onChange={(e) => { setName(e.target.value) }} />
                        </div>
                    </div>
                    ...
                    <div className='col-md-12 mb-4'>
                        <div className='border border-dark rounded-3 bg-white d-flex align-items-center px-3 h-auto'>
                            <i className='bi bi-pencil'></i>
                            <textarea className='border-0 rounded-3 p-2 w-100 px-3' placeholder='Enter about Product' defaultValue={about} onChange={(e) => { setAbout(e.target.value) }} />
                        </div>
                    </div>
